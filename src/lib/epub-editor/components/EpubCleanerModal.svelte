			...
			<div class="p-6 overflow-y-auto space-y-5">
				...
				{:else if editorState.cleanReport}
					...
					<div class="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-5 text-center">
						...
						<h4 class="font-mono text-base font-bold text-emerald-400">Dọn dẹp thành công!</h4>
						...
						<div
							class="text-left text-xs font-mono space-y-1 text-text-mute bg-panel/60 p-3 rounded-lg border border-border-color"
						>
							<div>
								• Ảnh đã xóa: <span class="text-text-color font-semibold"
									>{editorState.cleanReport.removedImages.length}</span
								>
							</div>
							<div>
								• Font đã xóa: <span class="text-text-color font-semibold"
									>{editorState.cleanReport.removedFonts.length}</span
								>
							</div>
							<div>
								• CSS đã xóa: <span class="text-text-color font-semibold"
									>{editorState.cleanReport.removedStyles.length}</span
								>
							</div>
							<div>
								• Mục Manifest đã cập nhật: <span class="text-text-color font-semibold"
									>{editorState.cleanReport.removedManifestEntries.length}</span
								>
							</div>
						</div>
					</div>
				{:else if editorState.cleanAnalysis}
					...
					<div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
						<div class="bg-sidebar-bg rounded-xl p-3 border border-border-color text-center">
							...
							<span class="font-mono text-sm font-bold text-text-color"
								>{formatByteSize(a.totalBytes)}</span
							>
						</div>
						<div class="bg-sidebar-bg rounded-xl p-3 border border-border-color text-center">
							<span class="text-xs text-text-mute block font-mono">Ảnh không dùng</span>
							<span
								class="font-mono text-sm font-bold {a.unusedImages.length > 0
									? 'text-amber-400'
									: 'text-text-color'}"
							>
						...
						<div class="bg-sidebar-bg rounded-xl p-3 border border-border-color text-center">
							...
							<span
								class="font-mono text-sm font-bold {a.unusedFonts.length > 0
									? 'text-amber-400'
									: 'text-text-color'}"
							>
					...
					<div class="space-y-3 bg-sidebar-bg p-4 rounded-xl border border-border-color">
						...
						<label class="flex items-start gap-3 cursor-pointer select-none">
							...
							<div>
								<div
									class="font-mono text-xs font-semibold text-text-color flex items-center gap-1.5"
								>
									...
									{#if a.unusedImages.length > 0}
										<span class="px-1.5 py-0.2 bg-amber-500/20 text-amber-400 text-[10px] rounded">
											{a.unusedImages.length} tệp (~{formatByteSize(
												a.unusedImages.reduce((s, i) => s + i.byteSize, 0)
											)})
										</span>
									{/if}
								</div>
							...
							<div>
								...
								>
									...
									{#if a.unusedFonts.length > 0}
										<span class="px-1.5 py-0.2 bg-amber-500/20 text-amber-400 text-[10px] rounded">
											{a.unusedFonts.length} tệp (~{formatByteSize(
												a.unusedFonts.reduce((s, f) => s + f.byteSize, 0)
											)})
										</span>
									{/if}
								</div>
					...
					{#if a.missingReferences.length > 0}
						<div
							class="bg-amber-500/10 border border-amber-500/30 rounded-xl p-3 text-xs font-mono"
						>
							<div class="text-amber-400 font-semibold mb-1">
								⚠️ Phát hiện {a.missingReferences.length} liên kết hỏng trong sách:
							</div>
							<div class="max-h-24 overflow-y-auto space-y-1 text-text-mute">
								{#each a.missingReferences.slice(0, 5) as ref (ref.sourceFile + '-' + ref.targetRef)}
									<div>
										• {ref.sourceFile} ➔ <span class="text-amber-300">{ref.targetRef}</span> (không tìm
										thấy)
									</div>
								{/each}
							</div>
				...
				{:else}
					<Button
						...
						disabled={isAnalyzing ||
							isOptimizing ||
							(editorState.cleanAnalysis?.estimatedSavingsBytes === 0 &&
								editorState.cleanAnalysis?.unusedImages.length === 0 &&
								editorState.cleanAnalysis?.unusedFonts.length === 0 &&
								editorState.cleanAnalysis?.unusedStyles.length === 0)}
					>
