  ...
  >
    ...
    >
      ...
      <div class="px-6 pt-4 pb-2 border-b border-border-color bg-panel/50 flex items-center gap-2">
        ...
        <button
          type="button"
          class="px-3 py-1.5 rounded-lg text-xs font-mono cursor-pointer transition-colors {selectedProfile === 'epub3'
            ? 'bg-accent-soft text-accent-color font-bold border border-accent-color/30'
            : 'bg-sidebar-bg text-text-mute hover:text-text-color border border-border-color'}"
        ...
        <button
          type="button"
          class="px-3 py-1.5 rounded-lg text-xs font-mono cursor-pointer transition-colors {selectedProfile === 'generic'
            ? 'bg-accent-soft text-accent-color font-bold border border-accent-color/30'
            : 'bg-sidebar-bg text-text-mute hover:text-text-color border border-border-color'}"
      ...
      <div class="p-6 overflow-y-auto space-y-5">
        ...
        {:else if validationResult}
          ...
          <!-- Summary Badges Card -->
          <div class="p-4 rounded-xl border {res.passed ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-red-500/10 border-red-500/30'}">
            <div class="flex items-center justify-between mb-3">
              <div class="flex items-center gap-2">
                <span class="text-xl">{res.passed ? '✅' : '⚠️'}</span>
                <div>
                  <h4 class="font-mono text-sm font-bold {res.passed ? 'text-emerald-400' : 'text-red-400'}">
                    {res.passed ? 'Tệp EPUB đạt chuẩn kiểm định!' : `Phát hiện ${res.errorCount} lỗi cần khắc phục`}
                  </h4>
                  <p class="text-xs text-text-mute font-mono">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-2 border-t border-border-color/40 text-xs font-mono">
              <div class="flex items-center gap-1.5 {res.summary.structure === 'pass' ? 'text-emerald-400' : 'text-red-400'}">
                <span>{res.summary.structure === 'pass' ? '✓' : '✗'}</span> Cấu trúc file
              </div>
              <div class="flex items-center gap-1.5 {res.summary.manifest === 'pass' ? 'text-emerald-400' : 'text-red-400'}">
                <span>{res.summary.manifest === 'pass' ? '✓' : '✗'}</span> Manifest
              </div>
              <div class="flex items-center gap-1.5 {res.summary.spine === 'pass' ? 'text-emerald-400' : 'text-red-400'}">
                <span>{res.summary.spine === 'pass' ? '✓' : '✗'}</span> Spine thứ tự
              </div>
              <div class="flex items-center gap-1.5 {res.summary.toc === 'pass' ? 'text-emerald-400' : 'text-red-400'}">
                <span>{res.summary.toc === 'pass' ? '✓' : '✗'}</span> Mục lục TOC
              </div>
              <div class="flex items-center gap-1.5 {res.summary.xhtml === 'pass' ? 'text-emerald-400' : 'text-red-400'}">
                <span>{res.summary.xhtml === 'pass' ? '✓' : '✗'}</span> Thẻ XHTML
              </div>
              <div class="flex items-center gap-1.5 {res.summary.fonts === 'pass' ? 'text-emerald-400' : 'text-amber-400'}">
                <span>{res.summary.fonts === 'pass' ? '✓' : '⚠'}</span> Phông chữ
              </div>
              <div class="flex items-center gap-1.5 {res.summary.cover === 'pass' ? 'text-emerald-400' : 'text-amber-400'}">
            ...
            <div class="space-y-2">
              ...
              <div class="space-y-2 max-h-72 overflow-y-auto pr-1">
                {#each res.issues as issue, idx (issue.category + '-' + (issue.file || '') + '-' + idx)}
                  <div class="p-3 rounded-xl border text-xs font-mono {issue.severity === 'error'
                    ? 'bg-red-500/10 border-red-500/30'
                    : issue.severity === 'warning'
                      ? 'bg-amber-500/10 border-amber-500/30'
                      : 'bg-blue-500/10 border-blue-500/30'}">
                    <div class="flex items-start gap-2">
                      <span class="shrink-0">{issue.severity === 'error' ? '🔴' : issue.severity === 'warning' ? '🟡' : 'ℹ️'}</span>
                      <div class="space-y-1 min-w-0">
                        <div class="font-semibold {issue.severity === 'error' ? 'text-red-400' : issue.severity === 'warning' ? 'text-amber-400' : 'text-blue-400'}">
                          {issue.message}
                        </div>
                        {#if issue.file}
