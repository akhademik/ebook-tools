...
function getOrCreateWorker(): Worker | null {
	...
	if (!workerInstance) {
		try {
			workerInstance = new Worker(new URL('./image-bg-remove.worker.ts', import.meta.url), {
				type: 'module'
			});
			...
			workerInstance.onmessage = (e: MessageEvent<WorkerResponse>) => {
				...
				if (data.type === 'progress') {
					...
					pendingJobs.delete(data.id);
					job.reject(new Error(data.error || 'Lỗi xử lý ảnh trong background worker'));
				}
			};
			...
			workerInstance.onmessageerror = (err: MessageEvent) => {
				...
				cleanupWorkerAndRejectPending(new Error('Lỗi truyền dữ liệu với Web Worker'));
			};
		} catch (err) {
			Logger.warn(
				'[ImageBgWorker]',
...
export async function removeImageBackgroundFallback(
	...
	if (typeof document === 'undefined') {
		return imageSource instanceof Blob ? imageSource : new Blob([], { type: 'image/png' });
	}
