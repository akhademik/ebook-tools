...
/**
 * Yield to the browser's event loop to allow UI repainting and smooth animations.
 */
 ...
 */
export async function loadImage(source: Blob | File | string): Promise<HTMLImageElement> {
	return new Promise((resolve, reject) => {
		...
		img.onload = () => {
			if (typeof source !== 'string' && typeof URL.revokeObjectURL === 'function') {
...
/**
 * Pre-downscales large images before running background removal.
 * Prevents UI freezes and drastically cuts down ML inference time and memory usage.
 */
 ...
 * Uses proxyToWorker to prevent blocking the main UI thread.
 */
	...
	const blob = await removeBackground(imageSource, {
		...
		progress: (key: string, current: number, total: number) => {
			...
			if (key.includes('fetch') || key.includes('download')) {
				...
				onProgress?.(`Đang làm phép thuật lên bức ảnh... (${percent}%)`, percent);
			} else {
...
export async function removeOrnamentBackground(
	...
	try {
		onProgress?.('Đang chuẩn bị làm phép thuật lên ảnh...', 5);
		await yieldToMain();

		// Pre-scale image if very large to prevent memory spikes and keep processing snappy
 ...
 * 1. Background removal (ML in WebWorker with Fallback)
...
export async function processOrnamentImage(
	...
	// 1. Remove background
	const transparentBlob = await removeOrnamentBackground(imageSource, onProgress);
	await yieldToMain();
