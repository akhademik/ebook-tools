...
function sendProgress(id: string, percent: number, statusText: string): void {
	self.postMessage({ id, type: 'progress', percent, statusText } as WorkerResponse);
}
...
self.onmessage = async (e: MessageEvent<WorkerRequest>) => {
	...
	try {
		sendProgress(id, 5, 'Đang chuẩn bị làm phép thuật lên ảnh...');
			...
			transparentBlob = await removeBackground(scaledBlob, {
				...
				progress: (key: string, current: number, total: number) => {
					...
					if (key.includes('fetch') || key.includes('download')) {
						...
						sendProgress(id, percent, `Đang làm phép thuật lên bức ảnh... (${percent}%)`);
					} else {
		...
		// 5. Convert to Blob
		const finalBlob = await canvas.convertToBlob({ type: 'image/png', quality: 0.92 });
