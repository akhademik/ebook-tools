...
<div class="modern-card rounded-2xl p-7 mb-6 animate-fade-in">
	...
	<div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
		<div>
			<Input bind:value={epubState.metadata.title} label="Tác phẩm" placeholder="Mật Mã Da Vinci" />
		</div>
		<div>
			<Input bind:value={epubState.metadata.author} label="Tác giả" placeholder="Dan Brown" />
		</div>
		<div>
			<Input
		...
		<div>
			<Input bind:value={epubState.jacket.translator} label="Dịch giả" placeholder="Đỗ Thu Hà" />
		</div>
		<div>
			<Input
	...
	>
		<div>
			<p class="font-mono text-xs text-text-mute uppercase tracking-wider font-semibold">
				Trang giới thiệu sách (Mặc định được tạo)
			</p>
			{#if selectedTemplateName}
				<p class="text-xs text-text-mute font-mono mt-1">
					Mẫu đang chọn: <span class="text-accent-color font-semibold">{selectedTemplateName}</span>
				</p>
			{/if}
		</div>
		<div>
