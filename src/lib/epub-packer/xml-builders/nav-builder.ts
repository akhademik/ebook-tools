...
function renderNcxNavPoints(nodes: TocNode[], startPlayOrder = 1): { xml: string; nextPlayOrder: number } {
  let playOrder = startPlayOrder;
  const parts: string[] = [];

  for (const node of nodes) {
    const currentOrder = playOrder++;
    let point = `<navPoint id="${escapeXml(node.id || `navPoint-${currentOrder}`)}" playOrder="${currentOrder}">\n` +
