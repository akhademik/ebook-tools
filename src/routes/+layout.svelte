...
>
	...
	>
		...
		<div class="flex items-center gap-3.5 mb-8">
			...
			<div>
				<h2 class="font-mono text-lg font-bold m-0 text-text-color tracking-tight">Ebook Maker</h2>
		...
		<nav class="flex flex-col gap-1.5 flex-1">
			...
			<a
				href="/epub-packer"
				onclick={closeMenu}
				class="tab-btn py-3 px-4 rounded-xl flex items-center gap-3 text-left transition-all duration-150 w-full border {$page.url.pathname.startsWith(
					'/epub-packer'
				)
					? 'bg-accent-soft text-accent-color font-semibold border-accent-color/30 shadow-xs'
					: 'text-text-mute font-medium border-transparent hover:text-text-color hover:bg-panel'}"
	...
	<main class="flex-1 flex flex-col h-full overflow-hidden">
		...
		>
			<div class="flex items-center gap-2.5">
				...
				<h2 class="font-mono text-base font-bold text-text-color">Ebook Maker</h2>
			</div>
			<button
				onclick={openMenu}
